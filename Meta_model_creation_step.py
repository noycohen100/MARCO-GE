...
import os
import pandas as pd
min_clustering_measures = ["DB", "HL","HKK", "Xie", "Scat", "average"]
def model(dir, depth, trees, l_r, col_s, sub_s, min_c, seed_v, number_of_features, training_file_name, number_of_algs, clustering_measure):
    ...
    rankings = pd.read_csv(training_file_name, index_col=0)
    datasets_counter = 0
    ...
    for dataset_test in datasets_list:
        ...
        if clustering_measure not in min_clustering_measures:
            prediction = -1*prediction
            y_test = -1*y_test
        prediction_rank = rankdata(prediction)
        y_r = rankdata(y_test, method='min')
        p_r = rankdata(prediction, method='min')
