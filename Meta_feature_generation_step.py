...
def retrive_graphs(dict_nodes_embedding, EdgeLists_folder, datasets_folder, label_file_name):
    #Graph_list = mp()
    Graph_list=[]
    for file in sorted(os.listdir(datasets_folder),key=lambda s: s.lower()):
        file = file.split('\\')[-1].split('.')[0]
        print(EdgeLists_folder)
...
def graphClassification(datasets_folder, EdgeLists_folder, NodesEmbedding_folder,number_of_epochs, embedding_size, num_classes,clustering_measure, labels_file):
    ...
    graphs= retrive_graphs(dict_nodes_embedding, EdgeLists_folder, datasets_folder, labels_file)
    loo = LeaveOneOut()
