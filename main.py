...
from Meta-feature_generation_step import graphClassification
from Meta-model_creation_step import model
...
if __name__ == '__main__':
    ...
    Graph_Representation(args.Df)
    Clustering_algorithm_evaluation(args.Df, args.em)
    label_file_name = merge_results.find_best_algorithm(args.mm)
    graphClassification(args.Df, EdgeLists_folder, NodesEmbedding_folder,args.ne, args.es, N_clustering_algs, args.mm,label_file_name)
    with open('XGBoostHyperParameters/'+args.mm +".json", "w") as f:
        parameters = json.load(f)
    training_file_name = merge_results.union_algorithms_performance(args.mm)
    model(args.DF, parameters['depth'],parameters['trees'], parameters['l_r'], parameters['col_s'], parameters['sub_s'], parameters['min_c'],parameters['seed'] ,args.es,training_file_name, N_clustering_algs )
